            ...
            try
            {
                if (!string.IsNullOrEmpty(modelTextbox.Text))
                {
                    using (SqlConnection db = new SqlConnection(conn))
                    {
                        ...
                        updateCommand.CommandText = "UPDATE headphone SET Model=@Model, Code_SN=@Code_SN, Received_date=@Received_date, Condition=@Condition, Remarks=@Remarks,OwnerID=@OwnerID WHERE hID='" + GlobalData.hID.ToString() + "'";
